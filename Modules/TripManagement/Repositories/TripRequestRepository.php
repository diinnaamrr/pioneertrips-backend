...
class TripRequestRepository implements TripRequestInterfaces
{
    ...
    public function store(array $attributes): Model
    {
        try {
            ...
            $trip->driver_id = $attributes['driver_id'] ?? null;
            $trip->vehicle_id = $attributes['vehicle_id'] ?? null;
            $trip->is_carpool = $attributes['is_carpool'] ?? false;
            $trip->carpool_route_id = $attributes['carpool_route_id'] ?? null;
            $trip->required_seats = $attributes['required_seats'] ?? null;
            $trip->required_bags = $attributes['required_bags'] ?? null;
            $trip->has_bags = $attributes['has_bags'] ?? false;
            $trip->save();
            ...
            $int_coordinates = $attributes['intermediate_coordinates'];
    ...
    public function getPendingRides($attributes): mixed
    {
        $query = $this->trip
            ...
            ->when($attributes['service'] ?? null, fn($query) => $query->whereIn('type', $attributes['service']))
            ->when($attributes['parcel_weight_capacity'] ?? null, function ($query) use ($attributes) {
                return $query->where(function ($query) use ($attributes) {
                    $query->whereHas('parcel', function ($query) use ($attributes) {
                        $query->where('weight', '<=', $attributes['parcel_weight_capacity']);
                    })->orDoesntHave('parcel');
                });
            })
