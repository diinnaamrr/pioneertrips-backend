...
class TripRequestController extends Controller
{
     ...
     * @return JsonResponse
     */
    public function rideResumeStatus(Request $request): JsonResponse
    {
        if ($request->type == 'carpool') {
             $trip = $this->getResumeRide(isCarpool: true);
        } else {
        ...
        $tripResource = TripRequestResource::make($trip);

        return response()->json(responseFormatter(constant: DEFAULT_200, content: $tripResource));
    }
    ...
    public function getEstimatedFare(Request $request): JsonResponse
    {
        ...
        if (!is_array($pickup_coordinates) || count($pickup_coordinates) < 2) {
        return response()->json(responseFormatter(DEFAULT_400, [
            'message' => 'Invalid pickup coordinates format.'
        ]), 400);
        }

        if (!is_array($destination_coordinates) || count($destination_coordinates) < 2) {
        return response()->json(responseFormatter(DEFAULT_400, [
   ...
   public function createRideRequest(Request $request): JsonResponse
    {
        // if($this->tripRequestRepository->getBy(criteria: ['customer_id' => auth('api')->id(), 'status' => 'in_progress','type'=>'carpool'])->count() > 0) {
        //     return response()->json(responseFormatter(INCOMPLETE_RIDE_403), 403);
        //     }

        // $trip = $this->getResumeRide(isCarpool: $request->type == 'carpool');
        ...
        $validator = Validator::make($request->all(), [
            ...
            // Other Fields
            'note' => 'sometimes',
            'type' => 'required|in:parcel,ride_request,carpool',
            ...
            'carpooling_route_id' => 'sometimes|numeric|exists:carpooling_routes,id',
            'required_seats' => 'nullable|integer|min:0',
            'required_bags' => 'nullable|integer|min:0',
            'has_bags' => 'nullable|boolean',


        ]);
        \Log::info('Create Ride Request Validation', $request->all());

        if ($validator->fails()) {
            return response()->json(responseFormatter(constant: DEFAULT_400, errors: errorProcessor($validator)), 403);
        }

        // Check if user has an active carpool trip (prevents creating any new trip)
        $activeCarpoolTrip = $this->tripRequestRepository->getBy(
            criteria: [
        ...
        if ($request->trip_request_id) {
            ...
            // Cast to float to ensure proper type
            $pickup_point = new Point((float)$pickup_coordinates[0], (float)$pickup_coordinates[1]);
            $destination_point = new Point((float)$destination_coordinates[0], (float)$destination_coordinates[1]);
        }
        ...
        $route = null;
        $snappedPickup = null;
        $carpoolRideLocationPoint = null;
        
        if($request->is_carpool == true) {
            if (!$request->carpool_route_id) {
            ...
            if (!$route || !$route->vehicle) {
                return response()->json(responseFormatter(DEFAULT_404, ['message' => 'Carpool route not found']), 404);
            }
            
            // Get route polyline and snap pickup point
            // Handle double-encoded JSON
            $routePointsRaw = $route->route_points;
            $routePolyline = [];
            
            if (!empty($routePointsRaw)) {
                // First decode to handle escaped quotes
                $decoded = json_decode($routePointsRaw, true);
                if (is_string($decoded)) {
                    // If it's still a string, decode again
                    $routePolyline = json_decode($decoded, true) ?? [];
                } else {
                    $routePolyline = $decoded ?? [];
                }
            }
            
            // Debug logging
            \Log::info('Carpool route debug', [
                'route_id' => $route->id,
                'route_points_raw' => $route->route_points,
                'routePolyline_decoded' => $routePolyline,
                'routePolyline_count' => count($routePolyline),
                'pickup_point' => ['lat' => $pickup_point->latitude, 'lng' => $pickup_point->longitude],
                'pickup_coordinates_array' => $pickup_coordinates
            ]);
            
            // Convert Point object to array format [lng, lat] for snapToRoutePolyline
            $pickup_coordinates = [$pickup_point->longitude, $pickup_point->latitude];
            $snappedPickup = $this->snapToRoutePolyline($pickup_coordinates, $routePolyline);
            
            // Debug logging for snapped pickup
            \Log::info('Snapped pickup debug', [
                'snappedPickup' => $snappedPickup,
                'snappedPickup_type' => gettype($snappedPickup),
                'snappedPickup_is_array' => is_array($snappedPickup)
            ]);
            
            $takenSeats = $this->tripRequestRepository->getBy(
                criteria: [
                    "carpool_route_id" => $request->carpool_route_id,
                    "status" => ACCEPTED
                ]
            )->count();
            if($route->seats_available <= $takenSeats) {
        ...
        if ($request->is_carpool == true && $route) {
            // For carpool trips, use the fixed price from the carpool route
            $estimatedFare = $route->price;
            $actualFare = $route->price;
            $riseRequestCount = 0;
            $returnFee = 0;
            $cancellationFee = 0;
        } elseif (array_key_exists('bid', $request->all()) && $request['bid']) {
        ...
        if ($request->trip_request_id) {
            ...
            // Validate array format
            if (!is_array($customer_coordinates) || count($customer_coordinates) < 2) {
                return response()->json(responseFormatter(DEFAULT_400, [
                    'message' => 'Invalid customer coordinates format.'
                ]), 400);
            }

            $customer_point = new Point((float)$customer_coordinates[0], (float)$customer_coordinates[1]);
            
            $carpoolRideLocationPoint = null;
            if ($request->is_carpool==true && $snappedPickup && is_array($snappedPickup) && isset($snappedPickup['lng']) && isset($snappedPickup['lat'])) {
                // Use DB::raw to create the Point directly in SQL format
                // MySQL POINT format is POINT(longitude latitude) - longitude first, then latitude
                $carpoolRideLocationPoint = DB::raw("ST_GeomFromText('POINT({$snappedPickup['lng']} {$snappedPickup['lat']})', 4326)");
            }
            
            // Debug logging for request merge
            if($request->is_carpool == true) {
                $carpoolRideLocationValue = null;
                if ($request->is_carpool==true && $snappedPickup && is_array($snappedPickup) && isset($snappedPickup['lng']) && isset($snappedPickup['lat'])) {
                    $carpoolRideLocationValue = 'Point(' . $snappedPickup['lng'] . ', ' . $snappedPickup['lat'] . ')';
                }
                
                \Log::info('Request merge debug', [
                    'is_carpool' => $request->is_carpool,
                    'snappedPickup_exists' => !empty($snappedPickup),
                    'snappedPickup' => $snappedPickup,
                    'snappedPickup_is_array' => is_array($snappedPickup),
                    'snappedPickup_has_lng_lat' => is_array($snappedPickup) && isset($snappedPickup['lng']) && isset($snappedPickup['lat']),
                    'carpool_ride_location_will_be' => $carpoolRideLocationValue,
                    'carpoolRideLocationPoint_created' => $carpoolRideLocationPoint ? 'yes' : 'no',
                    'carpoolRideLocationPoint_type' => $carpoolRideLocationPoint ? get_class($carpoolRideLocationPoint) : 'null',
                    'carpoolRideLocationPoint_sql' => $carpoolRideLocationPoint ? "ST_GeomFromText('POINT({$snappedPickup['lng']} {$snappedPickup['lat']})', 4326)" : null
                ]);
            }
            
            $request->merge([
                'customer_id' => auth('api')->id(),
                // 'customer_id' => '9e6965ca-e176-4a66-8c63-5fc5b0da29b7',
                'zone_id' => $zone->id,
                'pickup_coordinates' => $pickup_point,
                'destination_coordinates' => $destination_point,
                ...
                'carpooling_route_id' => $request->is_carpool==true ? $request->carpooling_route_id : null,
                'driver_id' => $request->is_carpool==true ? $route->user_id : null,
                'vehicle_id' => $request->is_carpool==true? $route->vehicle_id :null ,
                'is_carpool' => $request->is_carpool==true ? true : false,
                'required_seats' => $request->is_carpool==true ? $request->required_seats : null,
                'required_bags' => $request->is_carpool==true ? $request->required_bags : null,
                'vechicle_category_id' => $request->is_carpool==true? $route->vehicle->category_id :null,
                'encoded_polyline' => $request->is_carpool==true ? $route->encoded_polyline : ($request->encoded_polyline ?? null),
            ]);

            // Mostafa edits
            if ($request->is_carpool == true) {
                try {
                    $save_trip = DB::transaction(function () use ($request, $route) {
                        // Lock the route to avoid race conditions
                        $lockedRoute = CarpoolRoute::where('id', $route->id)->lockForUpdate()->firstOrFail();
                    
                        $requiredSeats = (int)($request->required_seats ?? 0);
                        $requiredBags = (int)($request->required_bags ?? 0);
                        $hasBags = filter_var($request->has_bags, FILTER_VALIDATE_BOOLEAN);
                    
                        $availableSeats = (int)$lockedRoute->seats_available;
                        $availableBags = (int)$lockedRoute->bags_available;
                        $maxBagsPerSeat = (int)$lockedRoute->bags_seats_available;
                    
                        //Case 1: Passenger booking seat(s)
                        if ($requiredSeats > 0) {
                            if ($availableSeats < $requiredSeats) {
                                throw new \Exception("Only {$availableSeats} seat(s) left.");
                            }
                    
                            if ($hasBags) {
                                $allowedBags = $requiredSeats * $maxBagsPerSeat;
                    
                                if ($requiredBags > $allowedBags) {
                                    throw new \Exception("You can only bring {$allowedBags} bag(s) for {$requiredSeats} seat(s).");
                                }
                    
                                if ($availableBags < $requiredBags) {
                                    throw new \Exception("Only {$availableBags} bag space(s) left.");
                                }
                    
                                $lockedRoute->bags_available -= $requiredBags;
                            }
                    
                            $lockedRoute->seats_available -= $requiredSeats;
                        }
                    
                        //Case 2: Sending bag(s) only
                        elseif ($requiredBags > 0 && $requiredSeats === 0) {
                            if ($availableBags < $requiredBags) {
                                throw new \Exception("Only {$availableBags} bag space(s) left.");
                            }
                        ...
                        return $this->trip->store(attributes: $request->all());
                    });
                    
                } catch (\Exception $e) {
                    return response()->json(responseFormatter(DEFAULT_400, [
                        'message' => $e->getMessage() ?: 'No seats or bags available for this route'
                    ]), 403);
                }
            ...
            // Set carpool_ride_location directly on the model if it's a carpool trip
            if($request->is_carpool == true && $carpoolRideLocationPoint) {
                $save_trip->carpool_ride_location = $carpoolRideLocationPoint;
                $save_trip->save();
            }
            
            // Debug logging after trip creation
            if($request->is_carpool == true) {
                \Log::info('Trip creation debug', [
                    'trip_id' => $save_trip->id,
                    'carpool_ride_location_stored' => $save_trip->carpool_ride_location,
                    'carpool_ride_location_type' => gettype($save_trip->carpool_ride_location),
                    'original_snappedPickup' => $snappedPickup
                ]);
            }
        }
        ...
        if (!empty($find_drivers)) {
            ...
            if (!empty($notify)) {
                ...
                // Send notifications directly instead of using the job
                foreach ($find_drivers as $driver) {
                    if ($driver->user?->fcm_token) {
                        try {
                            ...
                            \Log::error('Failed to send notification to driver', [
            ...
            foreach ($find_drivers as $key => $value) {
                try {
                    // TEST DISABLED: checkPusherConnection(CustomerTripRequestEvent::broadcast($value->user, $final));
    ...
    public function rideStatusUpdate($trip_request_id, Request $request): JsonResponse
    {
      ...
      //dina
          if ($request->status == 'cancelled' && $trip->type == 'carpool' && !empty($trip->carpool_route_id)) {
        $carpoolRoute = CarpoolRoute::find($trip->carpool_route_id);
        if ($carpoolRoute) {
            $carpoolRoute->seats_available += $trip->seats_booked ?? 1;
            $carpoolRoute->save();
        }
    }
      //
    ...
    public function finalFareCalculation(Request $request): JsonResponse
    {
        ...
        } elseif ($trip->type == 'carpool') {
            // For carpool trips, use the existing fare structure without complex calculations
            $fare = null;
            // Skip complex fare calculations for carpool trips
            $calculated_data = [
                'extra_fare_amount' => 0,
                'final_fare' => $trip->actual_fare,
        ...
        // Only calculate final fare for non-carpool trips
        if ($trip->type != 'carpool') {
        ...
        if (($bid_on_fare || $trip->rise_request_count > 0) && $trip->type == 'ride_request') {
            ...
            // Only apply discounts and coupons for non-carpool trips
    ...
    public function applyCoupon(Request $request): JsonResponse
    {
        ...
        if ($response['discount'] != 0) {
            ...
            try {
                // TEST DISABLED: checkPusherConnection(CustomerCouponAppliedEvent::broadcast($trip));
            } catch (Exception $exception) {
     ...
     */
    private function getIncompleteRide($isCarpool=false): mixed
    {
        ...
        if (!$trip ||  $trip->type != ($isCarpool ? 'carpool':'ride_request') ||
    ...
    private function getResumeRide($isCarpool=false): mixed
    {
        $attributes = [
            'type' => $isCarpool ? 'carpool' : 'ride_request',
            'column' => 'customer_id',
            'value' => auth()->id(),
        ];
      ...
      public function haversine($lat1, $lon1, $lat2, $lon2)
{
    ...
    private function decodePolyline($encoded)
    {
        ...
        while ($index < $len) {
            ...
            $poly[] = ['lat' => $lat / 100000, 'lng' => $lng / 100000];
        }
    ...
    /**
     * Snap a coordinate to the nearest point on a route polyline
     */
    private function snapToRoutePolyline($coordinate, $polyline)
    {
        if (empty($polyline)) {
            // Fallback to original coordinate if polyline is empty
            // $coordinate is in format [lng, lat] from the Point object
        ...
        foreach ($polyline as $point) {
            // $coordinate[0] is longitude, $coordinate[1] is latitude
        ...
        // Fallback to original if no closest point found
        // Ensure we return in the correct format: lng first, then lat
