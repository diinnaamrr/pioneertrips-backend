...
class TripRequestController extends Controller
{
    ...
    public function getCarpoolPassengers($carpool_route_id) : JsonResponse
    {

        $passengers = $this->carPoolRideOverview(
            carpool_route_id: $carpool_route_id,
        );
        return response()->json(responseFormatter(DEFAULT_200, $passengers));
     ...
     public function requestAction(Request $request): JsonResponse
    {
        ...
        if ($request['action'] != ACCEPTED) {
            ...
            // If this is a carpool trip that was rejected, decrement the counter if it was previously incremented
            if ($trip->carpool_route_id) {
                $cacheKey = "carpool_accepted_passengers_{$trip->carpool_route_id}";
        ...
        if ($trip->carpool_route_id) {
            $carpoolRoute = \Modules\TripManagement\Entities\CarpoolRoute::find($trip->carpool_route_id);

            if ($carpoolRoute) {

                $cacheKey = "carpool_accepted_passengers_{$trip->carpool_route_id}";

                $acceptedCount = Cache::increment($cacheKey, 1);

                Cache::put($cacheKey, $acceptedCount, now()->addDay());

                $totalExpectedPassengers = $this->getCarpoolTotalPassengers($trip->carpool_route_id);

                Log::info("Carpool passenger acceptance", [
                    'carpool_route_id' => $trip->carpool_route_id,
                    'accepted_count_cache' => $acceptedCount,
                    'total_expected_passengers' => $totalExpectedPassengers,
                    'driver_id' => $user->id,
                    'trip_id' => $trip->id
                ]);

                $actualAcceptedCount = $this->trip->getBy(
                    column: 'carpool_route_id',
                    value: $trip->carpool_route_id,
                    attributes: ['count' => true, 'column_name' => 'current_status', 'column_value' => 'accepted']
                );


                if ($acceptedCount >= $totalExpectedPassengers || $actualAcceptedCount >= $totalExpectedPassengers) {
                    $this->driverDetails->update(attributes: ['column' => 'user_id', 'availability_status' => 'on_trip'], id: $user->id);

                    Log::info("Driver status set to on_trip for carpool", [
                        'carpool_route_id' => $trip->carpool_route_id,
                        'driver_id' => $user->id,
                        'accepted_count_cache' => $acceptedCount,
                        'actual_accepted_count' => $actualAcceptedCount,
                        'total_expected' => $totalExpectedPassengers
                    ]);
                }
            } else {
                $this->driverDetails->update(attributes: ['column' => 'user_id', 'availability_status' => 'on_trip'], id: $user->id);
    ...
    public function rideStatusUpdate(Request $request): JsonResponse
    {
        ...
        if ($request->status == 'completed' || $request->status == 'cancelled') {
            ...
            // Clean up carpool passenger counter cache when trip is completed or cancelled
            if ($trip->carpool_route_id) {
                $cacheKey = "carpool_accepted_passengers_{$trip->carpool_route_id}";
    ...
    public function matchOtp(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            'trip_request_id' => 'required',
            'carpool_route_id' => 'nullable|exists:carpool_routes,id',
    ...
    public function pendingRideList(Request $request): JsonResponse
    {
        ...
        $pending_rides = $this->trip->getPendingRides(attributes: [
            ...
            'relations' => ['customer', 'ignoredRequests', 'time', 'fee', 'fare_biddings', 'parcel', 'parcelRefund'],
    ...
    private function carPoolRideOverview($carpool_route_id): mixed
    {
        return $this->trip->getBy(column: 'carpool_route_id', value: $carpool_route_id, attributes: [
            'relations' => ['customer', 'vehicleCategory', 'tripStatus', 'time', 'coordinate', 'fee', 'parcel', 'parcelUserInfo', 'parcelRefund'],
            'withAvgRelation' => 'customerReceivedReviews',
            'withAvgColumn' => 'rating'
        ]);
    ...
    /**
     * Get the total number of passengers expected for a carpool route
     *
     * @param string $carpoolRouteId
     * @return int
     */
    private function getCarpoolTotalPassengers(string $carpoolRouteId): int
    {
        // Get the carpool route
        $carpoolRoute = \Modules\TripManagement\Entities\CarpoolRoute::find($carpoolRouteId);

        if (!$carpoolRoute) {
            return 0;
        }

        // Count accepted passengers for this route
        $acceptedPassengers = $this->trip->getBy(
            column: 'carpool_route_id',
            value: $carpoolRouteId,
            attributes: ['count' => true, 'column_name' => 'current_status', 'column_value' => 'accepted']
        );

        // Count total passengers that have requested to join (accepted + pending)
        // $totalPassengerRequests = $this->trip->getBy(
        //     column: 'carpool_route_id',
        //     value: $carpoolRouteId,
        //     attributes: ['count' => true, 'column_name' => 'current_status', 'column_value' => ['accepted', 'pending']]
        // );

        // If we have a max_passengers field in the route, use it
        // Otherwise, calculate based on available seats + accepted passengers
        if (isset($carpoolRoute->seats_available) && $carpoolRoute->seats_available > 0) {
            return $carpoolRoute->seats_available;
        }

        // Fallback: assume total capacity is current accepted + available seats
        // This assumes seats_available represents the remaining available seats
        return $acceptedPassengers + $carpoolRoute->seats_available;
    }
