    ...
    $fixedPolyline = null;
            ...
            if ($trip->type === 'carpool' && !empty($trip->encoded_polyline)) {
                ...
                $tripData = [
                    ...
                    'type' => $trip->type,
                    'is_carpool' => $trip->is_carpool,
                ];
            }
            
            $route = getRoutes($pickup_coordinate, $drop_coordinate, $intermediate_coordinate, [$drivingMode], $fixedPolyline, $tripData);            $distance_in_km = $route[0]['distance'];
            ...
            $route = getRoutes($pickup_coordinate, $drop_coordinate, $intermediate_coordinate, [$drivingMode], $fixedPolyline, $tripData);
        ...
        $trip->required_seats > 1 ? $actual_fare = $actual_fare * $trip->required_seats : $actual_fare = $actual_fare;
    ...
    public function estimatedFare($tripRequest, $routes, $zone_id, $zone, $tripFare = null, $area_id = null, $beforeCreate = false): mixed
    {
        ...
        if ($tripRequest['type'] == 'parcel') {
            ...
            $estimated_fare = $tripFare->map(function ($trip) use ($routes, $tripRequest, $area_id, $beforeCreate, $zone) {
                ...
                $trip->required_seats > 1 ? $est_fare = $est_fare * $trip->required_seats : $est_fare = $est_fare;
                $discountFareVat = ($discountEstFare * $vat_percent) / 100;
                $discountEstFare += $discountFareVat;
                $vat = ($est_fare * $vat_percent) / 100;
                $est_fare += $vat;
