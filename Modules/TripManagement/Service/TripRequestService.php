...
class TripRequestService extends BaseService implements TripRequestServiceInterface
{
    ...
    public function storeTrip(array $attributes): Model
    {
        try {
            ...
            $tripData = [];
                ...
                $tripData['driver_id'] = isset($attributes['carpool_route_id']) && $attributes['carpool_route_id'] != null ? $attributes['driver_id'] : null;
                $tripData['vehicle_id'] = isset($attributes['carpool_route_id']) && $attributes['carpool_route_id'] != null ? $attributes['vehicle_id'] : null;
                $tripData['vehicle_category_id'] = isset($attributes['carpool_route_id']) && $attributes['carpool_route_id'] != null ? $attributes['vechicle_category_id'] : null;
                $tripData['encoded_polyline'] = $attributes['encoded_polyline'] ?? null;
                $tripData['carpool_route_id'] = $attributes['carpool_route_id'] ?? null;
                $tripData['current_status'] = $attributes['current_status'] ?? PENDING;
                $tripData['otp'] = $attributes['otp'] ?? null;
                $tripData['is_carpool'] = $attributes['carpool_route_id'] != null ? 1 : 0;
    ...
    {
        \Log::info('Make Ride Request', $request->request->all());
        ...
        if ($request->type == 'carpool') {
            ...
            $driver = $this->userRepository->findOneBy(criteria: ['id' => $request->driver_id]);
            if ($driver && $driver->fcm_token) {
                sendDeviceNotification(
                    ...
                    ride_request_id: $save_trip->id,
                    type: $save_trip->type,
                    action: 'carpool_ride_request_notification',
                    user_id: $driver->id
                );
            }
        } else {
            $search_radius = (float)get_cache('search_radius') ?? 5.0;
    ...
    /**
     * Validate and ensure fixed polyline for carpool trips
     * 
     * @param TripRequest $trip
     * @return bool
     */
    public function validateCarpoolPolyline($trip)
    {
        if ($trip->type !== 'carpool') {
            return true; // Not a carpool trip
        }

        // For carpool trips, ensure we have a fixed polyline
        if (empty($trip->encoded_polyline)) {
            // If no polyline exists, generate one based on current coordinates
            if ($trip->coordinate) {
                $pickupCoordinates = [
                ...
                if (isset($routes[0]['encoded_polyline'])) {
                    $trip->encoded_polyline = $routes[0]['encoded_polyline'];
                    $trip->save();
                    return true;
                }
            }
    ...
    /**
     * Get fixed polyline for carpool trip
     * 
     * @param string $tripId
     * @return string|null
     */
    public function getCarpoolFixedPolyline($tripId)
    {
        $trip = $this->findOne(id: $tripId);
        
        if (!$trip || $trip->type !== 'carpool') {
            return null;
        }
