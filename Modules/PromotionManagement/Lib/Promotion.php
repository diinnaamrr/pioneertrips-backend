...
if (!function_exists('sendDeviceNotification')) {
    ...
    {
        Log::info('==> sendDeviceNotification CALLED', [
            'fcm_token' => $fcm_token,
            'title' => $title,
        ...
        if ($user_id) {
            Log::info('Saving notification to database...', ['user_id' => $user_id]);
            ...
            Log::warning('No user_id provided. Notification will not be saved to database.');
        }

        $image = asset('storage/app/public/push-notification') . '/' . $image;
        Log::info('Image URL constructed.', ['image_url' => $image]);
        ...
        Log::info('Reward data extracted.', [
        ...
        Log::info('Final FCM postData constructed.', ['postData' => $postData]);

        $response = sendNotificationToHttp($postData);

        Log::info('Notification sent. Response:', ['response' => $response]);
