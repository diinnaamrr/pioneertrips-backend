        ...
        @forelse($otherData['trips'] as $key => $trip)
            <tr>
                ...
                <td class="text-center customer-name">
                    <a target="_blank" @if($trip->customer)href="{{route('admin.customer.show', ['id' => $trip->customer?->id])}}"@endif>
                ...
                <td class="text-center text-capitalize driver">
                    <a target="_blank"
                       @if($trip->driver && $trip->driver->id)
                           href="{{route('admin.driver.show', ['id' => $trip->driver->id])}}"
                        @endif
                    >
                        {{ $trip->driver?->id ? $trip->driver?->first_name. ' ' . $trip->driver?->last_name : translate('no_driver_assigned') }}
                    </a>
                </td>
                <td class="text-center trip-cost">{{ set_currency_symbol($trip->actual_fare ?? 0) }}</td>
                <td class="text-center coupon-discount">{{ set_currency_symbol($trip->coupon_amount ?? 0) }}</td>
                <td class="text-center text-capitalize additional-fee">
                    <div>{{translate('delay_fee')}}: {{ set_currency_symbol($trip->fee?->delay_fee ?? 0) }}</div>
                    <div>{{translate('idle_fee')}}: {{ set_currency_symbol($trip->fee?->idle_fee ?? 0) }}</div>
                    <div>{{translate('cancellation_fee')}}: {{ set_currency_symbol($trip->fee?->cancellation_fee ?? 0) }}</div>
                    <div>{{translate('Vat/Tax')}}: {{ set_currency_symbol($trip->fee?->vat_tax ?? 0) }}</div>
                </td>
                <td class="text-center total-trip-cost">{{ set_currency_symbol($trip->paid_fare ?? 0) }}</td>
                <td class="text-center admin-commission">{{ set_currency_symbol($trip->fee?->admin_commission ?? 0) }}</td>
