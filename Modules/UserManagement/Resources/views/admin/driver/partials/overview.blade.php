...
@php
    $docs = $driver?->identification_image ?? [];
@endphp
...
@forelse($docs as $doc)
    ...
    @if(in_array($ext, ['png','jpg','jpeg','gif']))
        <!-- Show image preview -->
        <a href="{{ $url }}" target="_blank" class="border rounded p-3 d-flex align-items-center gap-3">
            <img src="{{ $url }}" alt="{{ $doc }}" style="height:50px;">
            <h6 class="fs-12">{{ $doc }}</h6>
        </a>
    @else
        <!-- Show icon for non-images -->
        <a href="{{ $url }}" target="_blank" class="border rounded p-3 d-flex align-items-center gap-3">
            <i class="bi bi-file-earmark fs-22"></i>
            <h6 class="fs-12">{{ $doc }}</h6>
        </a>
    @endif
@empty
