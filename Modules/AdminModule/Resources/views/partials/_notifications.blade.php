        ...
        @forelse($notification as $notification)
            <a id="{{$notification->id}}" data-value="{{ $notification->id }}"
               @if($notification->message == 'new_customer_registered')
                   href="{{$notification->model_id ? route('admin.customer.show', ['id' => $notification->model_id]) : '#'}}"
               @elseif($notification->message == 'new_driver_registered')
                   href="{{$notification->model_id ? route('admin.driver.show', ['id' => $notification->model_id]) : '#'}}"
               @elseif($notification->model == 'trip_request')
                   href="{{$notification->model_id ? route('admin.trip.show', ['id' => $notification->model_id]) : '#'}}"
               @elseif($notification->model == 'channel_conversation')
                   href="{{route('admin.chatting')}}"
               @endif
               class="dropdown-item-text media gap-3 align-items-center seen-notification">
