...
class ConfigController extends Controller
{
    ...
    #
     ...
     public function getRoutes(Request $request)
     {
         ...
         // For carpool trips, use the fixed polyline stored in the trip_request table
         $fixedPolyline = null;
         if ($trip->type === 'carpool' && !empty($trip->encoded_polyline)) {
             $fixedPolyline = $trip->encoded_polyline;
         }
    ...
    public function geocodeApi(Request $request): JsonResponse
    {
        ...
        try {
            $response = Http::timeout(10)->get(MAP_API_BASE_URI . '/geocode/json?latlng=' . $request->lat . ',' . $request->lng . '&key=' . $mapKey);
            
            if (!$response->successful()) {
                return response()->json(responseFormatter(DEFAULT_400, null, null, null, ['Failed to connect to Google Maps API']), 400);
            }
            ...
            if (isset($data['status']) && $data['status'] !== 'OK') {
                $errorMessage = $data['error_message'] ?? 'Google Maps API error: ' . $data['status'];
                
                // Handle specific billing error
                if (str_contains($errorMessage, 'billing') || str_contains($errorMessage, 'REQUEST_DENIED')) {
                    $errorMessage = 'Google Maps API billing not enabled. Please contact administrator.';
                }
                
                return response()->json(responseFormatter(DEFAULT_400, null, null, null, [$errorMessage]), 400);
            }
            
            return response()->json(responseFormatter(DEFAULT_200, $data), 200);
            
        } catch (\Exception $e) {
            return response()->json(responseFormatter(DEFAULT_400, null, null, null, ['Geocoding service temporarily unavailable']), 400);
        }
    }
