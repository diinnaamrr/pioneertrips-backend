...
if (!function_exists('getRoutes')) {
    function getRoutes(array $originCoordinates, array $destinationCoordinates, array $intermediateCoordinates = [], array $drivingMode = ["DRIVE"], $fixedPolyline = null)
    {
        // If fixed polyline is provided (for carpool trips), use it instead of calculating new route
        if ($fixedPolyline && !empty($fixedPolyline)) {
            // Decode the fixed polyline to get distance and duration
            $apiKey = businessConfig(GOOGLE_MAP_API)?->value['map_api_key_server'] ?? '';
            
            // Use Google Directions API to get distance and duration for the fixed polyline
            ...
            if ($response->successful()) {
                ...
                if (isset($result['routes'][0]['legs'][0])) {
                ...
                $responses[0] = [
                    ...
                    'encoded_polyline' => $fixedPolyline,
                ];
        ...
        // Original logic for regular trips
        $apiKey = businessConfig(GOOGLE_MAP_API)?->value['map_api_key_server'] ?? '';
...
if (!function_exists('textVariableDataFormat')) {
                                    ...
                                    $userName = null, $withdrawNote = null, $paidAmount = null, $methodName = null, $referralRewardAmount = null, $otp = null)
    {
        ...
        if ($value) {
            ...
            if ($paidAmount) {
                $data = str_replace("{paidAmount}", $paidAmount, "");
            }
            if ($methodName) {
